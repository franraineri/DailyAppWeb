...
curl --header "Content-Type: application/json"   --request POST   --data '{"name":"first task",start_descrip: " Create the repository and add every one in the Team"}'   http://localhost:3000/task
curl --header "Content-Type: application/json"   --request POST   --data '{"name":"connections",start_descrip: " Make the conection between front and backend"}'   http://localhost:3000/task
curl --header "Content-Type: application/json"   --request POST   --data '{"name":"first object",start_descrip: " Create one object in the db"}'   http://localhost:3000/task
